...
- [Laragon](https://laragon.org/) (Apache + MySQL) atau XAMPP
...
1. **Clone repository**
   ```bash
   git clone https://github.com/superamarr/Teras-Samarinda.git
   cd Teras-Samarinda
   ```

2. **Setup database**
   - Buka phpMyAdmin atau MySQL client
   - Import file database (jika tersedia) ke database `terassamarinda`
   - Pastikan MySQL berjalan di Laragon
...
3. **Konfigurasi environment**
   ...
   ```
   Edit `.env` dan isi sesuai konfigurasi lokal:
   ```env
   VITE_API_BASE_URL=http://localPA.test/Teras-Samarinda/backend/public
   DB_HOST=localhost
   DB_NAME=terassamarinda
   DB_USER=root
   DB_PASS=
   ```

4. **Konfigurasi backend**
   - Pastikan virtual host di Laragon mengarah ke `backend/public/` sebagai document root
   - File `.htaccess` sudah dikonfigurasi untuk URL rewriting dan CORS
...
6. **Jalankan development server**
   ...
   npm run dev
   ```
   Frontend berjalan di `http://localhost:5173`, API request di-proxy ke backend Laragon.
...
Hasil build ada di folder `dist/` — berisi HTML, JS, CSS yang sudah dioptimasi dan siap di-deploy.

### Upload ke InfinityFree

1. Upload isi folder `dist/` ke `public_html/` di hosting
2. Upload seluruh folder `backend/` ke server (pastikan `backend/public/` sebagai document root)
3. Buat file `.env` di server dengan konfigurasi production:
   ```env
   VITE_API_BASE_URL=https://domain-anda.com/backend/public
   ```
4. Import database MySQL ke server
5. Pastikan `mod_rewrite` aktif di Apache untuk `.htaccess`

### Konfigurasi Production

Build production secara otomatis membaca dari `.env.production` (bukan `.env`), yang mengatur:
- API URL production (HTTPS)
- Cookie session dengan `Secure=true` dan `SameSite=None` untuk cross-origin
